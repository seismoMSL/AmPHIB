...
def MT_decomposition(MT):
    output = {'percentage':{},'elements':{}}
    MT_decomp = MomentTensor(M=MT).get_decomposition(style='p')
    
    output['percentage']['ISO'] = MT_decomp[5] 
    output['percentage']['DEV'] = MT_decomp[7] 
    output['percentage']['DC'] = MT_decomp[9] 
    output['percentage']['CLVD'] = MT_decomp[15] 
    
    DC_part = np.asarray(MT_decomp[8])
    output['elements']['DC'] = [DC_part[0][0],DC_part[1][1],DC_part[2][2],DC_part[0][1],DC_part[0][2],DC_part[1][2]]    
    CLVD_part = np.asarray(MT_decomp[14])
    output['elements']['CLVD'] = [CLVD_part[0][0],CLVD_part[1][1],CLVD_part[2][2],CLVD_part[0][1],CLVD_part[0][2],CLVD_part[1][2]]
    ISO_part = np.asarray(MT_decomp[4])
    output['elements']['ISO'] = [ISO_part[0][0],ISO_part[1][1],ISO_part[2][2],ISO_part[0][1],ISO_part[0][2],ISO_part[1][2]]
...
Pressure-axis'''.splitlines()
    ...
    def _build_gmt_dict(options, optparser):
        ...
        if float(temp_dict['GMT_scaling']) < epsilon:
            ...
            print( '%f - set to 1, due to obviously stupid input value' % (epsilon))
